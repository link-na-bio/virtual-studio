...
// Definição dos Pacotes e Preços (Mantenha os valores reais do seu negócio aqui)
const PACOTES_INFO: Record<string, { nome: string, preco: number, fotos: number, icon: any }> = {
  'basico': { nome: 'Básico', preco: 49.90, fotos: 20, icon: User },
  'popular': { nome: 'Popular', preco: 99.90, fotos: 50, icon: Star },
  'pro': { nome: 'Profissional', preco: 149.90, fotos: 100, icon: Zap }
};
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Verifica se o pedido pertence a quem está logado
        ...
        // 3. Buscar Fotos de Prévia (com URL Assinada)
  ...
  const handlePayment = async () => {
    ...
    setTimeout(async () => {
      try {
        // Atualiza o status no banco para liberar as fotos sem marca d'água
        ...
        router.push('/dashboard'); // Ou redirecionar para uma tela de sucesso
      } catch (error: any) {
  ...
  return (
    <div className="flex min-h-screen bg-studio-black text-white font-sans selection:bg-studio-gold selection:text-studio-black">
      ...
      {/* 🧭 SIDEBAR DO CLIENTE (Mesma da Dashboard) */}
      ...
      <main className="flex-1 p-8 overflow-y-auto bg-[#121212]">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          {/* Esquerda: Galeria de Fotos (Com Blindagem CSS) */}
            ...
            <div className="bg-white/5 backdrop-blur-xl p-8 rounded-2xl border border-studio-gold/20 relative overflow-hidden">
              ...
              <ul className="space-y-3 mb-8 text-sm text-gray-300 relative z-10">
                <li className="flex items-center gap-3">
                  ...
                  Até {pacoteInfo.fotos} fotos geradas em alta resolução
                </li>
                <li className="flex items-center gap-3">
                  <CheckCircle2 size={16} className="text-studio-gold" />
                  Sem marca d'água
